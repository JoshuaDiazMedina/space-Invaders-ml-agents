...
public class PlayerAgent : Agent
{
  ...
  public override void OnEpisodeBegin()
  {
    ...
    // Destruir l�ser existente antes de empezar episodios
  ...
  public override void CollectObservations(VectorSensor sensor)
  {
    ...
    {
      // Posición del bunker (110observaciones al space size)
    ...
    // Informaci�n de los misiles m�s cercanos (hasta 4)
    ...
    {
      sensor.AddObservation(0.0f); // Posici�n X
      sensor.AddObservation(Camera.main.orthographicSize / 2); // Posici�n Y
    }
  ...
  public override void OnActionReceived(ActionBuffers actions)
  {
    ...
    // Guardar posici�n anterior en X
    ...
    if (Mathf.Abs(position.x - previousX) < 0.01f)
    {
      AddReward(-0.005f); // Penalización leve si se queda estático
    }

    //float distanceMoved = Mathf.Abs(position.x - previousX);
    //AddReward(distanceMoved * 0.003f); // Aumentar recompensa si se mueve m�s
  ...
  private void OnTriggerEnter2D(Collider2D other)
  {
    ...
    {
      UnityEngine.Debug.Log("Me cay� un misil");
      SetReward(-1f);
      if (Academy.Instance.IsCommunicatorOn)
      {
        EndEpisode();
  ...
  {
    AddReward(-0.01f); // Penalizaci�n por destruir un bunker
  }
  public void OnLaserMissed()
  {
    UnityEngine.Debug.Log("Laser perdido");
    AddReward(-0.05f); // Penalizaci�n por disparar sin impactar un invasor
  }
  public void OnHitAllInvader()
  {
    UnityEngine.Debug.Log("Termin� con todos los invaders");
    SetReward(1f);
    EndEpisode();
  }
